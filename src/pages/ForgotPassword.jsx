  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 404) {
        // If email is not registered, user is redirected to the registration page with an appropriate message
        navigate("/signup", {
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 text-white h-full w-full">
          ...
          <div className="mb-12">
            ...
            <p className="text-lg text-gray-200 max-w-md leading-relaxed">
              We help you recover your access so you can get back to farming
              smarter.
            </p>
          </div>
        </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 md:p-12 animate-fade-in-up">
        <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full py-4 bg-[#2e7d32] hover:bg-[#1b5e20] text-white rounded-2xl font-bold text-lg shadow-lg hover:shadow-xl transition-all duration-300 transform cursor-pointer hover:-translate-y-0.5 active:translate-y-0"
            >
              Next
            </button>
          </form>
          ...
          <p className="text-center mt-10 text-gray-500">
            ...
            <Link
              to="/login"
              className="text-[#2e7d32] font-semibold cursor-pointer hover:underline"
            >
              Log in
            </Link>
          </p>
